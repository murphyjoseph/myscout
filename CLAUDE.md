...
make save URL=https://... OUTCOME=good_shot  # save a job posting
...
- **`docs/architecture.md`** — system diagrams, data flow, and tool/dependency summary. Update when the system topology changes (new services, data flow, infrastructure).
- **`docs/dependencies.md`** — every dependency with what it does, why it's here, and how we use it. **Update when adding or removing any package** — every dependency must justify its existence.
- **`docs/features.md`** — comprehensive list of all features and their status. Update when adding, removing, or changing features.
- **`docs/roadmap.md`** — planned work, ideas, and completed milestones. Move items to "Completed" with a date when done.
- **`docs/logs.md`** — auto-maintained dev log. A PostToolUse hook appends a one-line entry after edits. You may also add entries manually when completing significant work.

When making changes, keep these docs in sync: if you add a feature, update `features.md`; if you add a dependency, update `dependencies.md`; if you change architecture, update `architecture.md`; if you complete a roadmap item, move it to "Completed" in `roadmap.md`.
